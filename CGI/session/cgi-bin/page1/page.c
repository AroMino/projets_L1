...
void print_link_nav(int total_number,Info* info){
	...
	for(int i=0 ; i<page_n ; i++){
		if(info->p == i) printf("<a class=\"actual_numero\" href=\"/cgi-bin/page?user=%s&p=%d\">%d</a>",info->user,i,i+1);
		else printf("<a class=\"numero\" href=\"/cgi-bin/page?user=%s&p=%d\">%d</a>",info->user,i,i+1);
	}
