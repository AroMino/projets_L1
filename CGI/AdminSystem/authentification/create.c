...
int main(){
	...
	if(analyse_login(login,error)){
		...
		fprintf(file,"%s %s\n",login,password);
		fclose(file);
		printf("Location: http://www.adminsystem.mg/authentification/sign_up_success.cgi?%s\n\n",login);
	}
	
	free(login);
	free(password);
	printf("Location: http://www.adminsystem.mg/authentification/sign_up.cgi?error_sign_up=%s\n\n",error);
